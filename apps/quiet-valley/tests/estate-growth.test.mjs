...
test('estate scene requires explicit terrain bindings instead of creating satellite pads',()=>{
 const R={group:()=>({visible:true}),add:()=>({visible:true})};
 const BaseWorld={make:()=>({roots:{farm:{}},sync(){},inspect(){},focusCamera(){}})};
 assert.throws(()=>createEstateWorld(BaseWorld,{}).make(R,{}),/terrain bindings/);
 // Full real-mesh growth and preservation are covered by radial-land.test.mjs.
});
