...
'use strict';
export function createValleyUI(FarmSim, FarmExpansion) {
 ...
 function estatePanel(s){
  ...
  let html=`<section class="estate-panel"><div class="land-region-title">🏝️ ${st.meta.name}<span>размер ${st.meta.area}% · площадь ${st.areaPercent}% · ${st.tier}/4</span></div><div class="notice"><b>Остров растёт по всему кругу. Новая земля доступна для строительства.</b><br>Нажмите «Выбрать место», затем на зелёную клетку в мире. Постройки можно бесплатно перемещать. Штат: ${e.staff.length}/${st.staffCapacity} · крупные постройки: ${e.buildings.length}/${st.buildingCapacity}.</div>`;
  if(next){const c=next.cost,disabled=s.coins<c.coins||s.world.materials.wood<c.wood||s.world.materials.stone<c.stone;html+=`<article class="project-card estate-growth"><span class="project-icon">🗺️</span><div class="project-copy"><h3>Расширить остров по кругу · ${next.area}%</h3><p>${next.name}. Откроется: ${next.unlocks.join(', ')}.</p><div class="project-meta">${cost(c)}</div></div><button class="primary" data-action="expandEstate" ${disabled?'disabled':''}>Расширить</button></article>`;}else html+=`<div class="notice">✓ Максимальная территория открыта: ${st.meta.area}%. Теперь развитие идёт через здания и команду.</div>`;
  ...
  for(const [key,b] of Object.entries(X.estateBuildings||{})){
   ...
   html+=`<article class="project-card ${built?'built':''}"><span class="project-icon">${b.icon}</span><div class="project-copy"><h3>${b.name}</h3><p>${b.desc}</p><div class="project-meta">${built?'✓ Построено':cost(b.cost)}</div>${locked?`<small class="project-blocker">Нужен уровень острова ${b.tier}</small>`:full?'<small class="project-blocker">Нет свободного строительного слота — расширьте остров</small>':''}</div><button class="primary" data-place="estate:${key}" ${disabled?'disabled':''}>${built?'Переместить':'Выбрать место'}</button></article>`;
  }
 ...
 function modal(kind,s){let html=header(s,kind);const region=s.world.region;
  if(kind==='map'){
   ...
   html+=`</div><div class="modal-button-row"><button class="secondary" data-place="remove">Разобрать украшения · вернуть 50%</button></div><div class="fineprint">На этом участке: ${s.world.decor.filter(d=>d.region===region).length} / ${X.decorCapacity(s)} украшений. Постройки и природные деревья не разбираются этим инструментом.</div>`;
  }
  return html;
 }
