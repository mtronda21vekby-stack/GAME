...
export function createFarming(clock){
 ...
 function fresh(now=clock.now()){let plots=Array.from({length:16},(_,id)=>({id,unlocked:id<12,crop:null,plantedAt:0,waterAt:0,readyAt:0}));
 ...
 function validate(raw,now=clock.now()){
 ...
 function moisture(p,now=clock.now()){if(!p||!p.unlocked||!p.waterAt)return 0;return p.crop?1:clamp(1-(now-p.waterAt)/EMPTY_WET_MS,0,1);}
 function status(p,now=clock.now()){if(!p.unlocked)return 'locked';if(!p.crop)return 'empty';if(!p.waterAt)return 'dry';return now>=p.readyAt?'ready':'growing';}
 function act(s,action,now=clock.now()){if(!action||typeof action!=='object')return {ok:false,message:'Некорректное действие'};tick(s,now);let p=s.plots[action.id],a=s.animals.find(a=>a.id===action.id);const fail=message=>({ok:false,message});
  switch(action.type){
