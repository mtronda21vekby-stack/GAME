...
export function createStory(X, clock) {
 ...
 function weather(s,now=clock.now()){
 ...
 function ensureOrders(s,now=clock.now()){
 ...
 function rushActive(order,now=clock.now()){return !!order&&now<=order.bonusUntil;}
 function orderPreview(s,order,now=clock.now()){
 ...
 S.fresh=function(now=clock.now()){const s=base.fresh(now);s.version=4;s.game=gameDefaults(now);ensureOrders(s,now);return s;};
