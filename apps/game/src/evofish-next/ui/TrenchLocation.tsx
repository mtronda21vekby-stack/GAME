...
function readState(): TrenchState { if (typeof window === "undefined") return {}; try { const raw = window.localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) as TrenchState : {}; } catch { return {}; } }

const NODES = [
  { title: "Край разлома", status: "Открыто", reward: "Жемчуг, опыт, монеты", danger: "Высокая" },
  { title: "Кристальные ребра", status: "Скоро", reward: "Ancient Crystal", danger: "Очень высокая" },
  { title: "Гнездо бездны", status: "Скоро", reward: "Древний сундук", danger: "Элитная" },
    ...
    <main className="efTrench">
      ...
      <section className="efTrenchShell">
        <header className="efTrenchHeader">
          ...
          <aside><b>Аккаунт: {save.account.level}</b><b>Персонаж: {save.progress.level}</b><b>Ключи: {format(trenchKeys)}</b><b>Кристаллы: {format(state.ancientCrystals || 0)}</b></aside>
        </header>

        {!accessAllowed ? (
          <section className="efTrenchLocked">
            <div>🔒</div>
            <h2>ВПАДИНА закрыта</h2>
