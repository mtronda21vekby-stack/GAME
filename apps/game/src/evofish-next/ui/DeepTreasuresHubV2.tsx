...
type TrenchState = { diamonds: number; ancientCrystals: number; trenchKeys: number; deepTrenchUnlocked: boolean; history: { id: string; label: string; amount: number; status: string; tone: Tone }[]; skins: Record<string, boolean>; dryKey: number; drySkin: number };
...
function applyReward(base: TrenchState, reward: Reward) {
  ...
  let state = { ...base, skins: { ...base.skins } };
...
export function DeepTreasuresHub() {
  ...
  const [message, setMessage] = useState("Древняя рулетка ВПАДИНЫ готова.");
  const [pending, setPending] = useState<Reward | null>(null);
  const [reel, setReel] = useState<Reward[]>(() => buildReel(readState()));
  ...
  function spin() {
    ...
    const next = { ...fresh, dryKey: reward.kind === "trenchKey" ? 0 : fresh.dryKey + 1, drySkin: reward.kind === "veryRareSkin" || reward.kind === "mythicSkin" ? 0 : fresh.drySkin + 1 };
    ...
    setTimeout(() => { setPhase("done"); setPending(reward); setMessage(`Выпало: ${reward.label}`); }, 3550);
  }
  function claim() {
    if (!pending || rolling) return;
    const result = applyReward(readState(), pending);
    commit({ ...result.state, history: [{ id: id(), label: pending.label, amount: amount(pending), status: "Забрано", tone: pending.tone }, ...result.state.history].slice(0, 30) });
  ...
  function pickChest(i: number) {
    ...
    setOpenChest(i);
    if (i === failChest) { commit({ ...readState(), history: [{ id: id(), label: pending.label, amount: amount(pending), status: "Сгорело", tone: pending.tone }, ...readState().history].slice(0, 30) }); setPending(null); return setMessage("Награда сгорела."); }
    const next = { ...pending, multiplier: Math.min(16, (pending.multiplier || 1) * 2) };
    setPending(next); setFailChest(null); setMessage(`Успех. Множитель x${next.multiplier}`);
  }
  function openTrench() {
