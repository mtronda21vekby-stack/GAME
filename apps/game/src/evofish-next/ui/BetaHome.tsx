...
function statusLabel(report: EvoFishSaveDoctorReport) {
  ...
  if (report.status === "reset") return "Run сброшен";
  return "Ошибка сохранения";
}

function statusTone(report: EvoFishSaveDoctorReport) {
  if (report.status === "healthy" || report.status === "repaired") return "good";
      ...
      <section className="efBetaHomeShell">
        <header className="efBetaHomeHero">
          ...
          <div className={`efSaveStatus ${statusTone(doctor)}`}>
            ...
            <b>{statusLabel(doctor)}</b>
            <em>{doctor.issues[0] || "Ошибок не найдено."}</em>
          </div>
        </header>

        <section className="efBetaHomeStats">
          <article><span>Уровень run</span><b>LV {save.progress.level}</b><small>Tier {save.progress.tier} · Mass {save.progress.mass.toFixed(2)}</small></article>
          <article><span>Кошелёк</span><b>🦪 {format(save.economy.pearls)}</b><small>💎 {format(save.economy.corals)}</small></article>
          <article><span>Прогресс</span><b>{mutations} мутаций</b><small>{ownedSkins} скинов · {achievements} достижений</small></article>
