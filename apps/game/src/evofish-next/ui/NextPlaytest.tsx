...
const VIEW_SETTINGS_KEY = "evofish_next_view_settings_v4";
...
const UI_TEXT = {
  ru: {
    ...
    settings: {
      ...
      language: "Язык",
      languageHint: "Переключает основной интерфейс игры, обучение и настройки.",
      russian: "Русский",
      english: "English",
      stickTuning: "Настройка стика",
      fixed: "Фиксированный",
      floating: "Плавающий",
      stickSize: "Размер стика",
      ...
      balanced: "Баланс",
      high: "Высокое",
      qualityHint: "Низкое качество снижает DPR/визуальную нагрузку. Высокое оставляет максимум визуала.",
      autoZoom: "Авто-зум",
      manualZoom: "Ручной зум",
      wide: "Шире",
      close: "Ближе",
      tutorial: "Обучение",
      restartTutorial: "Повторить обучение",
      tutorialHint: "Заново покажет первые 5 подсказок."
    },
    tutorial: [
      ...
      { title: "4/5 · Задания", body: "Открывай Меню → Задания. Daily/Weekly дают новые цели и награды каждый день и каждую неделю." },
  ...
  en: {
    ...
    settings: {
      ...
      stick: "Stick",
      gamepad: "Gamepad",
      touchHint: "Touch: drag your finger toward the direction you want to move.",
      stickHint: "Stick works even in Lock UI. Stick type can be fixed or floating.",
      gamepadHint: "Gamepad: left stick moves, A/RT bites, B/RB/LT dashes.",
      language: "Language",
      languageHint: "Switches the main game UI, tutorial and settings.",
      russian: "Русский",
      english: "English",
      stickTuning: "Stick Tuning",
      fixed: "Fixed",
      floating: "Floating",
      stickSize: "Stick Size",
      sensitivity: "Sensitivity",
      stickHint2: "Fixed stays bottom-left. Floating appears where you touch the left control zone.",
      quality: "Quality",
      low: "Low",
      balanced: "Balanced",
      high: "High",
      qualityHint: "Low reduces DPR/visual load. High keeps maximum visuals.",
      autoZoom: "Auto Zoom",
      ...
      tutorialHint: "Shows the first 5 hints again."
    },
    tutorial: [
      ...
      { title: "4/5 · Quests", body: "Open Menu → Quests. Daily/Weekly goals give new rewards every day and week." },
  ...
  return (
    <main ref={rootRef} className={`efNextPlay ${uiLocked ? "locked" : ""}`} onContextMenu={(event) => event.preventDefault()}>
      ...
      {!uiLocked && activePanel ? (
        <div className={`efGamePanel ${activePanel === "map" ? "mapPanel" : ""}`}>
          <div className="efPanelHead">
            <b>{panelTitle(activePanel, viewSettings.language)}</b>
            <button onClick={() => setActivePanel(null)}>×</button>
          </div>

          {activePanel === "menu" ? (
            <div className="efMenuGrid">
              <button onClick={() => setActivePanel("craft")}>{copy.menu.craft}</button>
              <button onClick={() => setActivePanel("mutations")}>{copy.menu.mutations}</button>
              <button onClick={() => setActivePanel("quests")}>{copy.menu.quests}</button>
              <button onClick={() => setActivePanel("shop")}>{copy.menu.shop}</button>
              <button onClick={() => setActivePanel("settings")}>{copy.menu.settings}</button>
              <button onClick={lockUi}>{copy.menu.lock}</button>
            </div>
          ) : null}
          ...
          {activePanel === "map" ? (
            <div className="efFullMapPanel">
              ...
              <div className="efMapLegend">
                <span><b className="player" />{copy.map.player}</span>
                <span><b className="apex" />{copy.map.apex}</span>
                <span><b className="hunter" />{copy.map.hunter}</span>
                <span><b className="event" />{copy.map.event}</span>
              </div>
            </div>
          ) : null}
          ...
          {activePanel === "craft" ? NEXT_CRAFT_RECIPES.map((recipe) => {
            ...
            return (
              <button key={recipe.id} className="efPanelItem" disabled={!canUse} onClick={() => craftRecipe(recipe.id)}>
                ...
                <em>{recipe.duration ? `${recipe.duration} ${copy.rewards.seconds}` : copy.rewards.instant}</em>
              </button>
            );
          }) : null}
          ...
          {activePanel === "mutations" ? NEXT_MUTATIONS.map((mutation) => {
            ...
            return (
              <button key={mutation.id} className="efPanelItem" disabled={!canBuy} onClick={() => buyMutationLevel(mutation.id)}>
                ...
                <em>{liveLevel >= mutation.maxLevel ? "MAX" : `${mutation.coralCost} ${copy.rewards.crystal}`}</em>
              </button>
            );
          }) : null}
          ...
          {activePanel === "quests" ? NEXT_QUESTS.map((quest) => {
            ...
            return (
              <div key={quest.id} className={`efQuestItem ${done ? "done" : ""}`}>
                ...
                <em>{copy.rewards.reward}: {quest.reward.xp} XP · {quest.reward.pearls} {copy.shop.pearls}{quest.reward.corals ? ` · ${quest.reward.corals} ${copy.rewards.crystal}` : ""}</em>
              </div>
            );
          }) : null}
          ...
          {activePanel === "settings" ? (
            <div className="efSettingsPanel">
              <div className="efSettingBlock">
                <b>{copy.settings.language}</b>
                <div className="efControlModes two">
                  <button className={viewSettings.language === "ru" ? "active" : ""} onClick={() => setViewSettings((current) => ({ ...current, language: "ru" }))}>{copy.settings.russian}</button>
                  <button className={viewSettings.language === "en" ? "active" : ""} onClick={() => setViewSettings((current) => ({ ...current, language: "en" }))}>{copy.settings.english}</button>
                </div>
                <small>{copy.settings.languageHint}</small>
              </div>
              <div className="efSettingBlock">
                <b>{copy.settings.control}</b>
                <div className="efControlModes">
                  <button className={viewSettings.controlMode === "pointer" ? "active" : ""} onClick={() => setControlMode("pointer")}>{copy.settings.touch}</button>
                  <button className={viewSettings.controlMode === "joystick" ? "active" : ""} onClick={() => setControlMode("joystick")}>{copy.settings.stick}</button>
                  <button className={viewSettings.controlMode === "gamepad" ? "active" : ""} onClick={() => setControlMode("gamepad")}>{copy.settings.gamepad}</button>
                </div>
                <small>{viewSettings.controlMode === "joystick" ? copy.settings.stickHint : viewSettings.controlMode === "gamepad" ? copy.settings.gamepadHint : copy.settings.touchHint}</small>
              </div>

              {viewSettings.controlMode === "joystick" ? (
                <div className="efSettingBlock">
                  <b>{copy.settings.stickTuning}</b>
                  <div className="efControlModes two">
                    <button className={viewSettings.stickMode === "fixed" ? "active" : ""} onClick={() => { resetStick(); setViewSettings((current) => ({ ...current, stickMode: "fixed" })); }}>{copy.settings.fixed}</button>
                    <button className={viewSettings.stickMode === "floating" ? "active" : ""} onClick={() => { resetStick(); setViewSettings((current) => ({ ...current, stickMode: "floating" })); }}>{copy.settings.floating}</button>
                  </div>
                  <label className="efZoomControl"><span>{copy.settings.stickSize}: {Math.round(viewSettings.stickSize)}px</span><input type="range" min="76" max="132" step="2" value={viewSettings.stickSize} onChange={(event) => setViewSettings((current) => ({ ...current, stickSize: Number(event.currentTarget.value) }))} /></label>
                  <label className="efZoomControl"><span>{copy.settings.sensitivity}: {viewSettings.stickSensitivity.toFixed(2)}x</span><input type="range" min="0.65" max="1.55" step="0.05" value={viewSettings.stickSensitivity} onChange={(event) => setViewSettings((current) => ({ ...current, stickSensitivity: Number(event.currentTarget.value) }))} /></label>
                  <small>{copy.settings.stickHint2}</small>
                </div>
              ) : null}

              <div className="efSettingBlock">
                <b>{copy.settings.quality}</b>
                <div className="efControlModes">
                  <button className={viewSettings.quality === "low" ? "active" : ""} onClick={() => setViewSettings((current) => ({ ...current, quality: "low" }))}>{copy.settings.low}</button>
                  <button className={viewSettings.quality === "balanced" ? "active" : ""} onClick={() => setViewSettings((current) => ({ ...current, quality: "balanced" }))}>{copy.settings.balanced}</button>
                  <button className={viewSettings.quality === "high" ? "active" : ""} onClick={() => setViewSettings((current) => ({ ...current, quality: "high" }))}>{copy.settings.high}</button>
                </div>
                <small>{copy.settings.qualityHint}</small>
              </div>

              <div className="efSettingRow">
                <span>{copy.settings.autoZoom}</span>
              ...
              <label className="efZoomControl">
                <span>{copy.settings.manualZoom}: {viewSettings.zoom.toFixed(2)}x</span>
              ...
              <div className="efZoomPresets">
                <button onClick={() => setZoom(0.62)}>{copy.settings.wide}</button>
                <button onClick={() => setZoom(0.82)}>{copy.settings.balanced}</button>
                <button onClick={() => setZoom(1.06)}>{copy.settings.close}</button>
              </div>
              <div className="efSettingBlock">
                <b>{copy.settings.tutorial}</b>
                <button className="efWideButton" onClick={resetTutorial}>{copy.settings.restartTutorial}</button>
                <small>{copy.settings.tutorialHint}</small>
              </div>
            </div>
          ) : null}
          ...
          {activePanel === "shop" ? (
            <div className="efShopPanel">
              <div className="efShopBalance">
                <span><b>🦪</b>{formatNumber(stats.pearls)}<em>{copy.shop.pearls}</em></span>
                <span><b>💎</b>{formatNumber(stats.corals)}<em>{copy.shop.crystals}</em></span>
              </div>
                ...
                {shopSkins.map((skinDef) => {
                  ...
                  return (
                    <button key={skinDef.id} className={`efShopCard ${equipped ? "equipped" : ""} ${skinDef.unlock.type === "currency" && skinDef.unlock.currency === "corals" ? "coralPrice" : ""}`} disabled={!canAct} onClick={() => applySkinAction(skinDef.id)}>
                      ...
                      <b>{skinDef.name}</b>
                      <small>{skinPriceLabel(skinDef, viewSettings.language)}</small>
                      <em>{label}</em>
                    </button>
                  );
                })}
              </div>
              <Link to="/game/next/skins">{copy.shop.fullSkinLab}</Link>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {showJoystick && viewSettings.stickMode === "floating" ? (
        <div className="efStickFloatZone" onPointerDown={startFloatingStick} onPointerMove={moveFloatingStick} onPointerUp={releaseStick} onPointerCancel={releaseStick}>{!stick.active ? <span>{copy.controls.floatingZone}</span> : null}</div>
      ) : null}
      ...
      <div className="efNextControls">
        <button disabled={downed} onPointerDown={(event) => { event.preventDefault(); inputRef.current.bite = true; }}>{copy.controls.bite}</button>
        <button disabled={downed} onPointerDown={(event) => { event.preventDefault(); inputRef.current.dash = true; }}>{copy.controls.dash}</button>
      </div>
      {!uiLocked ? (
        <div className="efQuickBar">
          <button onClick={() => togglePanel("map")}>{copy.controls.map}</button>
          <button className="primary" onClick={() => togglePanel("menu")}>{copy.controls.menu}</button>
          <button onClick={lockUi}>{copy.controls.lock}</button>
        </div>
      ) : (
        <button className="efUnlockPill" onClick={() => setUiLocked(false)}>{copy.controls.unlock}</button>
      )}
      <style>{`
