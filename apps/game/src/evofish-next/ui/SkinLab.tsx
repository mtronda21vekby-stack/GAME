    ...
    <main className="efNextLab">
      <header className="efNextTopbar">
        <div>
          ...
          <h1>Skin Lab</h1>
          <p>Первый слой нового движка: data-driven скины, формы, palette и pattern без поломки текущей игры.</p>
        </div>
          ...
          return (
            <button key={skin.id} className={`efSkinCard ${skin.id === selected.id ? "active" : ""}`} onClick={() => setSelectedId(skin.id)}>
              ...
              <div className="efSkinCardBody">
                <b>{skin.name}</b>
                <span>{rarityLabel(skin.rarity)} · {priceLabel(skin)}</span>
              </div>
            </button>
          );
        })}
