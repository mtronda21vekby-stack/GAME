...
export function SkinLab() {
  ...
  const actionText = equipped ? "Надето" : owned ? "Надеть" : canBuy ? "Купить и надеть" : "Недостаточно валюты";
    ...
    <main className="efNextLab">
      <header className="efNextTopbar">
        <div>
          ...
          <h1>Skin Lab</h1>
          <p>Первый слой нового движка: скины уже имеют Next-save, покупку и экипировку без поломки текущей игры.</p>
        </div>
      ...
      <section className="efFormTabs" aria-label="Формы">
        {FORM_ORDER.map((id) => {
          ...
          return (
            <button key={id} className={id === form ? "active" : ""} onClick={() => setForm(id)}>
              <b>{def.name}</b>
              <span>{getSkinsForForm(id).length} skins</span>
            </button>
          );
        })}
      </section>
      ...
      <section className="efLabStage">
        ...
        <div className="efHeroInfo">
          ...
          <button className="efPrimaryAction" disabled={equipped || (!owned && !canBuy)} onClick={applySelectedSkin}>{actionText}</button>
      ...
      <section className="efSkinGrid">
        {skins.map((skin) => {
          ...
          return (
            <button key={skin.id} className={`efSkinCard ${skin.id === selected.id ? "active" : ""}`} onClick={() => setSelectedId(skin.id)}>
              <SkinPreview skin={skin} form={previewForm} size="sm" />
              <div className="efSkinCardBody">
                <b>{skin.name}</b>
                <span>{cardEquipped ? "Надето" : cardOwned ? "Куплен" : priceLabel(skin)}</span>
              </div>
            </button>
          );
        })}
