...
import { queueCraftAction, readCraftInventorySnapshot } from "../systems/craftSystem";
...
export function CraftQuickOverlay() {
  ...
  const [wallet, setWallet] = useState(() => loadEvoFishNextSave().economy);
  ...
  const refresh = () => {
    ...
    setWallet(loadEvoFishNextSave().economy);
  };
    ...
    <section className={`efCraftQuick ${open ? "open" : ""}`}>
      ...
      {open ? (
        <div className="efCraftDrawer">
          ...
          <p>Покупай усиления заранее. Они лежат в инвентаре и активируются вручную во время забега.</p>
