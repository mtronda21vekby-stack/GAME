...
const ROADMAP: Item[] = [
  ...
  { title: "Онлайн функции", desc: "Комнаты, чат, статусы и совместные события.", tag: "Social" },
  { title: "Магазин", desc: "Витрина скинов и тем, единая валюта и косметика.", tag: "Store" }
];

const CHANGELOG: Item[] = [
  { title: "Премиум UI", desc: "Стеклянные панели, адаптив и быстрые анимации.", tag: "UI" },
  { title: "Навигация", desc: "Внутренние страницы без перезагрузки, внешние apps — отдельно.", tag: "Routing" },
  { title: "PWA", desc: "Manifest + иконки, корректные meta для iOS.", tag: "PWA" }
];

function Card(props: { item: Item; art: string }) {
  const it = props.item;
  return (
    <div className="glassStrong bc-motion" style={{ borderRadius: 22, overflow: "hidden" }}>
      <div style={{ padding: 16 }}>
        ...
        <div style={{ marginTop: 8, color: "rgba(255,255,255,0.74)", fontWeight: 750, lineHeight: 1.45 }}>{it.desc}</div>
      </div>
      <div style={{ padding: 12, paddingTop: 0 }}>
        <img alt="" src={props.art} style={{ width: "100%", height: 150, objectFit: "cover", borderRadius: 16, display: "block" }} />
      </div>
    </div>
  );
}
    ...
    <main className="bcSiteRoot">
      <section className="bcHero">
        ...
        <header className="bcTop">
          ...
          <div className="bcRight">
            ...
            <Button variant="primary" leftIconSrc={Icons.play} onClick={() => window.location.assign("/game/")}>Играть</Button>
          </div>
        </header>
      ...
      <section className="bcSection">
        ...
        <div className="bcCards">
          <Card item={ROADMAP[0]} art={HeroArt.cardNeon} />
          <Card item={ROADMAP[1]} art={HeroArt.cardWave} />
          <Card item={ROADMAP[2]} art={HeroArt.cardGrid} />
        </div>
      </section>
      ...
      <section className="bcSection">
        ...
        <div className="bcCards">
          <Card item={CHANGELOG[0]} art={HeroArt.cardGrid} />
          <Card item={CHANGELOG[1]} art={HeroArt.cardWave} />
          <Card item={CHANGELOG[2]} art={HeroArt.cardNeon} />
        </div>
      </section>
    </main>
  );
