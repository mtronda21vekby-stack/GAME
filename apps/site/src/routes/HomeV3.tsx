...
export function HomeV3() {
  ...
  const [statusSnapshot, setStatusSnapshot] = React.useState<BlackCrownWorldStatusSnapshot>(
    FALLBACK_STATUS_SNAPSHOT
  );
  ...
  const evofish =
    statusSnapshot.statuses.evofish ?? BLACKCROWN_WORLD_STATUS_FALLBACK.evofish;
  const crownFront =
    statusSnapshot.statuses["crown-front"] ?? BLACKCROWN_WORLD_STATUS_FALLBACK["crown-front"];
  const network =
    statusSnapshot.statuses["blackcrown-network"] ??
    BLACKCROWN_WORLD_STATUS_FALLBACK["blackcrown-network"];

  const heroStatuses = React.useMemo<HeroStatus[]>(
    () => [
      {
        label: "EVOFISH",
        value: evofish.status,
        tone: toHeroTone(evofish.tone),
      },
      {
        label: "CROWN//FRONT",
        value: crownFront.status,
        tone: toHeroTone(crownFront.tone),
      },
      {
        label: "BLACKCROWN NETWORK",
        value: network.status === "LIVE" ? "ONLINE" : network.status,
        tone: toHeroTone(network.tone),
      },
    ],
  ...
  const feedTitle =
    statusSnapshot.source === "supabase"
      ? "LIVE STATUS"
  ...
  const feedDetail =
    statusSnapshot.source === "supabase"
      ? `SUPABASE · ${formatSyncTime(statusSnapshot.syncedAt)}`
      : statusSnapshot.source === "cache"
      ...
      <section id="worlds" className="bcWorldsV2" aria-labelledby="bc-worlds-v2-title">
        <GlassSurface material="frosted" tone="cyan" className="bcWorldsV2__intro">
          <div className="bcWorldsV2__head">
            ...
            <div
              className="bcWorldsV2__statusFeed"
              data-source={statusSnapshot.source}
              aria-live="polite"
              title={`${feedTitle}: ${feedDetail}`}
            >
              <i aria-hidden="true" />
              <strong>{feedTitle}</strong>
              <small>{feedDetail}</small>
            </div>
