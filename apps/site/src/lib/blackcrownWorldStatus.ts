...
// Dedicated BlackCrown project. This publishable key is safe for browser use;
// Row Level Security keeps public visitors read-only.
const SUPABASE_URL = "https://nmgotvxisujvpfoxivoq.supabase.co";
const SUPABASE_PUBLISHABLE_KEY = "sb_publishable_B2dxtgWZBYfC8iRkV02k0Q_FDJ01QNR";
...
function readCachedRows(): CachedWorldStatusPayload | null {
  ...
  try {
    ...
    if (parsed.version !== 1 || typeof parsed.syncedAt !== "number" || !Array.isArray(parsed.rows)) {
      return null;
    }
...
function writeCachedRows(rows: BlackCrownWorldStatus[], syncedAt: number) {
  ...
  try {
    ...
    // Storage may be blocked in privacy mode. The live request still works.
  }
}
...
export async function loadBlackCrownWorldStatuses(
  signal?: AbortSignal
): Promise<BlackCrownWorldStatusSnapshot> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`BlackCrown status request failed: ${response.status}`);
    }

    const payload = (await response.json()) as unknown;
    if (!Array.isArray(payload)) {
      throw new Error("BlackCrown status response is not an array");
    }
    ...
    if (rows.length === 0) {
      throw new Error("BlackCrown status response is empty");
    }
