  ...
  return (
    <section className="bcHero bcV3Hero bcHeroConcept" aria-labelledby="bc-hero-concept-title">
      ...
      <header className="bcHeroConcept__bar">
        <button type="button" className="bcHeroConcept__brand" onClick={() => onNavigate("/")} aria-label="BlackCrown — главная">
          <span className="bcHeroConcept__brandMark"><img src={Icons.crown} alt="" width="30" height="30" /></span>
        ...
        <button type="button" className="bcHeroConcept__playTop" onClick={onPlay}>
          <span>Играть</span><i aria-hidden="true">›</i>
        </button>
      </header>

      <div className="bcHeroConcept__copy">
        <div className="bcHeroConcept__network">
          <i aria-hidden="true" /><span>BLACKCROWN NETWORK</span><strong>{network}</strong>
        </div>
        <h1 id="bc-hero-concept-title" className="bcHeroConcept__title">BLACKCROWN</h1>
        <p className="bcHeroConcept__tagline">Одна <span>корона.</span> Несколько <strong>миров.</strong></p>
        ...
        <div className="bcHeroConcept__actions">
          <button type="button" className="bcHeroConcept__cta bcHeroConcept__cta--primary" onClick={onPlay}>
            <i aria-hidden="true" /><span>Запустить BlackCrown</span>
          </button>
          <button type="button" className="bcHeroConcept__cta bcHeroConcept__cta--secondary" onClick={onExploreWorlds}>Смотреть миры</button>
        </div>
      </div>

      <div className="bcHeroConcept__art" aria-hidden="true">
        <div className="bcHeroConcept__artAura" />
        <img className="bcHeroConcept__artImage" src="/art/hero-crown.jpg" alt="" fetchPriority="high" />
        <div className="bcHeroConcept__artTopMask" />
      </div>

      <div className="bcHeroConcept__sceneCode" aria-hidden="true">
        <span>SCENE / 00</span><strong>CROWN CORE</strong><span>DEPTH LINK ACTIVE</span>
      </div>

      <button type="button" className="bcHeroConcept__scroll" onClick={onExploreWorlds} aria-label="Перейти к мирам BlackCrown">
        <i aria-hidden="true" /><span>SCROLL</span>
      </button>
    </section>
  );
}
