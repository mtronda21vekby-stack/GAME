...
export function evaluateExperienceTimeline(progress: number, reducedMotion = false): ExperienceTimelineState {
  ...
  // 0–6%: core awakening. 6–22%: dense mechanical/nano assembly.
  const assembly = reducedMotion ? 1 : smootherstep(getRangeProgress(p, 0.055, 0.22));

  // 22–30%: clean hero inspection before the Crown physically opens and exits.
  ...
  // Crown opens for the camera passage, settles closed out of view, then opens
  // again for the final 96–100% pass-through.
