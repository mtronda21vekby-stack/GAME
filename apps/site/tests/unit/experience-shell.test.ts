...
describe("Experience Skeleton story", () => {
  ...
  it("maps cinematic progress and spatial hashes deterministically", () => {
    expect(getStoryProgress(0.31)).toMatchObject({ chapterId: "world-gate", localProgress: expect.closeTo(0.0769, 3) });
    expect(getStoryProgress(0.85).chapterId).toBe("network-core");
    ...
    expect(getTransitionProgress(0.31, [0.27, 0.35])).toBeCloseTo(0.5, 5);
  });
  ...
  it("keeps one scene active outside transitions and at most two inside", () => {
    ...
    const transition = resolveSceneLifecycle(0.31);
    ...
    expect(resolveSceneLifecycle(0.48).activeSceneIds).toEqual(["evofish-abyss"]);
  });
  ...
  it("evaluates registry transitions from absolute progress without drift", () => {
    ...
    registry.evaluate(0.31, 1, false, "high");
    ...
    registry.evaluate(0.31, 3, false, "high");
