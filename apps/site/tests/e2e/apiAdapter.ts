...
export async function installApiAdapter(page: Page, options: ApiAdapterOptions = {}): Promise<ApiAdapterState> {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/auth/guest") return response(route, 200, { ok: true, userId: "e2e-user", profile: { id: "e2e-user" } });
    if (url.pathname === "/api/commerce/entitlements") {
      ...
      return response(route, 200, { ok: true, entitlements: { userId: "e2e-user", itemIds: [...state.entitlements], updatedAt: Date.now(), source: "server" } });
    }
    if (url.pathname === "/api/commerce/quote") {
      const body = request.postDataJSON() as { items: TestLine[] };
      const quote = validateLines(body.items);
      return quote ? response(route, 200, { ok: true, quoteId: "quote-e2e", currency: "BC", ...quote }) : response(route, 400, { ok: false, reason: "invalid_cart" });
    }
    if (url.pathname === "/api/commerce/checkout") {
      ...
      const now = Date.now();
      const order: TestOrder = { id: orderId, userId: "e2e-user", status: "fulfilled", paymentMethod: "mock", currency: "BC", items: quote.items, total: quote.total, createdAt: now, fulfilledAt: now };
      state.orders.set(orderId, order);
      idempotency.set(key, orderId);
      order.items.forEach((item) => state.entitlements.add(item.itemId));
...
export function makeOrder(id = "ord-valid"): TestOrder {
  ...
  const now = Date.now();
  return {
    id, userId: "e2e-user", status: "fulfilled", paymentMethod: "mock", currency: "BC",
    items: [{ itemId: item.id, title: item.title, quantity: 1, unitPrice: item.price, lineTotal: item.price }],
    total: item.price, createdAt: now, fulfilledAt: now,
  };
}
