...
// Production-safe: no network requests, no runtime asset paths.
// Any missing icon key falls back to crown to avoid Safari "?" broken-image placeholders.

const CROWN_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" role="img" aria-label="Black crown favicon">
  <defs>
...
function toDataUri(svg: string): string {
  // keep it stable & Safari-safe
  const compact = svg.replace(/\s+/g, " ").trim();
  return `data:image/svg+xml;utf8,${encodeURIComponent(compact)}`;
}

const crown = toDataUri(CROWN_SVG);

// пока — один стиль на всё (как ты просил).
// если позже захочешь отдельный play/telegram — добавим отдельные SVG.
const baseIcons = {
  crown,
  play: crown,
} as const;

// Proxy: Icons.anything -> crown (чтобы нигде не было "?" в проде)
export const Icons: Record<string, string> = new Proxy(baseIcons as any, {
  get(target, prop) {
